...
--OR : 출판사 대한미디어 또는 이상미디어 출판한 책 전체 조회
SELECT *
  FROM BOOK
...
SELECT * FROM BOOK
 WHERE PRICE NOT BETWEEN 8000 AND 22000 --경계값 제외
 ORDER BY PRICE
;
-------------------
--책제목이 '야구' ~ '올림픽'
...
;
-----------------
--책제목에 '구' 문자가 있는 책 목록 조회
SELECT * FROM BOOK
 WHERE BOOKNAME LIKE '%구%';
--책제목에 '구' 문자가 2번째에 있는 책 조회
----UPDATE BOOK SET BOOKNAME = '우리 야구의 역사' WHERE BOOKID=7;
SELECT * FROM BOOK
 WHERE BOOKNAME LIKE '_구%';
--책제목 중 2,3번째 글자가 '구의'인 책 목록
SELECT * FROM BOOK

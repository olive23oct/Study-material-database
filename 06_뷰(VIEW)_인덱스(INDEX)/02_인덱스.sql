...
--WHERE 절에 인덱스가 만들어진 컬럼으로 조회할 때 인덱스가 사용됨
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE = 8000;
--WHERE 절의 비교순서가 바뀌어도 인덱스를 사용할 수 있으면 사용해서 처리
SELECT * FROM BOOK WHERE PRICE = 8000 AND PUBLISHER = '굿스포츠';

DROP INDEX IX_BOOK; 
--IX_BOOK 삭제 처리 후
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠'; --IX_BOOK2 인덱스 적용
SELECT * FROM BOOK WHERE PRICE = 8000; --인덱스적용 없이 FULL SCAN

--범위검색 : 인덱스 사용됨
SELECT * FROM BOOK WHERE PUBLISHER BETWEEN '굿스포츠' AND '대한미디어';

--LIKE 조건 사용시 인덱스 적용여부
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%'; --인덱스 적용
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어';--인덱스 사용안함
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%';--인덱스 사용안함
--------------------
--원본 데이타를 가공처리 하는 순간 인덱스 적용할 수 없음
